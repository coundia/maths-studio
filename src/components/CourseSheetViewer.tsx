...
export const CourseSheetViewer: React.FC<CourseSheetViewerProps> = ({
  ...
  return (
    <div id="course-sheet-viewer" className="flex flex-col space-y-6">
      ...
      <div className="rounded-2xl p-5 sm:p-6 bg-gradient-to-br from-indigo-950/70 via-slate-900/80 to-emerald-950/60 border border-indigo-500/30 shadow-xl backdrop-blur-md">
          ...
          <div className="flex items-center space-x-2">
            {onOpenPrerequisites && (
              <button
                ...
                className="px-3 py-1.5 rounded-xl text-xs font-bold bg-slate-800 hover:bg-slate-700 text-amber-300 border border-amber-500/30 transition-all flex items-center space-x-1.5"
              >
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition-all flex items-center space-x-1.5"
            >
          ...
          <div className="mt-4 pt-4 border-t border-indigo-500/20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
              {content.keyFormulas.map((f, i) => (
                <div
                  key={i}
                  className="bg-slate-950/80 border border-indigo-500/30 rounded-xl p-2.5 flex flex-col justify-between shadow-xs"
                >
                  <span className="text-[11px] font-semibold text-slate-400 mb-1">{f.label}</span>
                  <div className="font-mono text-xs sm:text-sm text-emerald-300 overflow-x-auto py-0.5">
                      ...
                      {method.solutionSteps.map((step, idx) => (
                        ...
                        >
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className="text-[10px] font-mono text-neutral-400">Étape {idx + 1}</span>
                          </div>
