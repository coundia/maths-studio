      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-3xl max-h-[85vh] shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="p-5 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {/* Cache Hit Rate */}
            <div className="bg-slate-100/80 dark:bg-slate-950/80 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
              <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 mb-1">
                <span>Taux Cache Hit</span>
                <span className="text-[11px] font-mono text-amber-600 dark:text-amber-400 font-medium">Cible ≥ 60%</span>
              </div>
            ...
            <div className="bg-slate-100/80 dark:bg-slate-950/80 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
              <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 mb-1">
                ...
                <span className="text-[11px] font-mono text-emerald-600 dark:text-emerald-400 font-medium">Cible ≥ 25%</span>
              </div>
            ...
            <div className="bg-slate-100/80 dark:bg-slate-950/80 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
              <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 mb-1">
                ...
                <span className="text-[11px] font-mono text-indigo-600 dark:text-indigo-400 font-medium">Cible ≤ 1.0</span>
              </div>
            ...
            <div className="bg-slate-100/80 dark:bg-slate-950/80 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
              <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 mb-1">
                ...
                <span className="text-[11px] font-mono text-sky-600 dark:text-sky-400 font-medium">Cible ≥ 70%</span>
              </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100 flex items-center">
                <Database className="w-4 h-4 mr-1.5 text-indigo-600 dark:text-indigo-400" />
            ...
            {history.length === 0 ? (
              ...
              <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden divide-y divide-slate-200/80 dark:divide-slate-800/80 bg-slate-100/60 dark:bg-slate-950/60 max-h-64 overflow-y-auto">
                {history.map((sol) => (
                  <div
                    key={sol.id}
                    onClick={() => {
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="text-slate-500 dark:text-slate-400 font-mono text-[11px] flex items-center">
                        <Clock className="w-3 h-3 mr-1" />
                        {sol.latencyMs}ms
                      </span>
                      <span className="text-slate-600 dark:text-slate-400 font-mono text-[10px] hidden sm:inline" title={sol.id}>
                        {sol.id.slice(0, 8)}...
                      </span>
                    </div>
                  </div>
        ...
        {/* Modal Footer */}
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-100/80 dark:bg-slate-950/80 flex items-center justify-between text-xs text-slate-600 dark:text-slate-400">
          <span>Architecture conforme PRD V1 : Zéro redondance & Stockage atomique</span>
          <button
            onClick={onClose}
            className="px-4 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white rounded-lg transition-colors"
          >
            Fermer
          </button>
        </div>
