...
export const StepPlayer: React.FC<StepPlayerProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl p-5 lg:p-6 shadow-xl justify-between">
      {/* Header with step number & rule badge */}
      <div>
        <div className="flex items-center justify-between pb-3.5 border-b border-slate-200/80 dark:border-slate-800/80 mb-4">
          <div className="flex items-center space-x-2">
            <span className="px-2.5 py-1 rounded-md bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 font-mono text-xs font-semibold uppercase tracking-wider border border-indigo-500/30">
              Étape {currentStepIndex + 1} sur {steps.length}
            </span>
            <span className="text-slate-500 dark:text-slate-400 text-xs">•</span>
            <span className="text-slate-700 dark:text-slate-300 text-xs font-mono font-medium">
              {solution.rawExpression}
            </span>
          </div>

          <div className="flex items-center space-x-1.5 bg-slate-100/80 dark:bg-slate-800/80 text-emerald-600 dark:text-emerald-400 px-2.5 py-1 rounded-md text-xs font-medium border border-emerald-500/20">
        ...
        {/* Step Title */}
        <h3 className={`font-bold text-slate-900 dark:text-white tracking-tight ${isClassroomMode ? 'text-2xl' : 'text-lg lg:text-xl'} mb-2`}>
          {currentStep.title}
        </h3>

        {/* Active KaTeX Formula Box */}
        <div className="bg-slate-100/90 dark:bg-slate-950/90 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 my-3 shadow-inner overflow-x-auto text-center flex flex-col items-center justify-center">
          <div className="text-[11px] uppercase tracking-wider text-slate-600 dark:text-slate-400 font-mono mb-1">
            Formule mathématique à cette étape
          </div>
          <div className={`${isClassroomMode ? 'text-2xl py-2' : 'text-xl py-1'} text-slate-900 dark:text-slate-100`}>
            <MathView latex={currentStep.latex} display={true} />
          </div>
        </div>

        {/* Concrete "Terre-à-terre" Explanation Card */}
        <div className="mt-3.5 p-3.5 rounded-xl bg-gradient-to-br from-indigo-50/50 dark:from-indigo-950/50 to-slate-50/60 dark:to-slate-950/60 border border-indigo-500/25">
          <div className="text-xs font-bold text-indigo-700 dark:text-indigo-300 uppercase tracking-wider mb-1 flex items-center">
            <Blocks className="w-4 h-4 mr-1.5 text-amber-600 dark:text-amber-400" />
            {concreteNote.title}
          </div>
        ...
        <div className="mt-3 text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
          <span className="text-slate-700 dark:text-slate-300 font-medium">Règle géométrique : </span>
      ...
      {isLastStep && (
        <div className="my-3 p-3.5 rounded-xl bg-gradient-to-r from-emerald-50/60 dark:from-emerald-950/60 to-indigo-50/60 dark:to-indigo-950/60 border border-emerald-500/30 flex items-center space-x-3">
          ...
          <div>
            <div className="font-semibold text-emerald-700 dark:text-emerald-300 text-xs sm:text-sm">
              Preuve géométrique 3D terminée !
            </div>
            <div className="text-xs text-slate-700 dark:text-slate-300">
              Forme factorisée : <span className="font-mono text-slate-900 dark:text-white font-bold">{solution.finalFormLatex}</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="flex items-center justify-between space-x-3">
          <button
            ...
            className={`flex-1 flex items-center justify-center space-x-2 py-2.5 px-4 rounded-xl font-medium text-xs sm:text-sm transition-colors ${
              isFirstStep
                ? 'bg-slate-100/50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 cursor-not-allowed border border-slate-200 dark:border-slate-800'
